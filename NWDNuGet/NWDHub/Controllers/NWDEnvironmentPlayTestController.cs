...
public class NWDEnvironmentPlayTestEditionController : NWDModelEditionAsyncController<NWDProjectPlayTestDataTrack>
{
    private const NWDEnvironmentKind _ENVIRONMENT_KIND = NWDEnvironmentKind.PlayTest;
    private const int _TRACK_LENGHT = 2;
    private const int _TRACK_MIN = 200;
    ...
    public override void PrepareBeforeEdit(NWDProjectPlayTestDataTrack? sObject, Dictionary<string, string>? sDictionary, HttpContext sHttpContext)
    {
        ulong tProjectRef = 0;
    ...
    {
        Func<NWDProjectPlayTestDataTrack, bool> tTest = (sData => sData.Reference > 0);
        if (sDictionary.ContainsKey(nameof(NWDProjectSubObject.Project)))
        {
    ...
    public override void NewAddon(NWDProjectPlayTestDataTrack sObject, Dictionary<string, string> sValues, HttpContext sHttpContext)
    {
        ...
        if (tProject != null)
        {
            ...
            List<NWDProjectPlayTestDataTrack> tList = NWDWebDataManager.GetAllData<NWDProjectPlayTestDataTrack>(sHttpContext);
            ...
            {
                tTestTrack = true;
                foreach (NWDProjectPlayTestDataTrack tObject in tList)
                {
    ...
    public override void UpdateAddon(NWDProjectPlayTestDataTrack sObject, Dictionary<string, string> sValues, HttpContext sHttpContext)
    {
        ulong tProjectReference = ulong.Parse(sValues[nameof(NWDProjectSubObject.Project)]);
        NWDProject? tProject = NWDWebDataManager.GetDataByReference<NWDProject>(sHttpContext, tProjectReference);
        if (tProject != null)
        {
            ...
            NWDProjectPlayTestDataTrack? tOldObject = NWDWebDataManager.GetDataByReference<NWDProjectPlayTestDataTrack>(sHttpContext, sObject.Reference);
    ...
    public override void DeleteAddon(NWDProjectPlayTestDataTrack sObject, Dictionary<string, string> sValues, HttpContext sHttpContext)
    {
        ulong tProjectReference = ulong.Parse(sValues[nameof(NWDProjectSubObject.Project)]);
        NWDProject? tProject = NWDWebDataManager.GetDataByReference<NWDProject>(sHttpContext, tProjectReference);
        if (tProject != null)
