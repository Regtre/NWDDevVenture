...
public class NWDPlayerClassConstructionEditionController : NWDModelEditionAsyncController<NWDPlayerClassConstruction>
{
    #region Virtual methods to filter

    protected override Func<T, bool> Filter<T>(Dictionary<string, string> sDictionary)
    {
        Func<NWDPlayerClassConstruction, bool> tTest = (sData => sData.Reference > 0);
    ...
    public override void NewAddon(NWDPlayerClassConstruction sObject, Dictionary<string, string> sValues, HttpContext sHttpContext)
    {
    ...
    public override void UpdateAddon(NWDPlayerClassConstruction sObject, Dictionary<string, string> sValues, HttpContext sHttpContext)
    {
        if (sValues.ContainsKey(nameof(NWDProjectSubObject.Project)))
        {
            ulong tProjectReference = ulong.Parse(sValues[nameof(NWDProjectSubObject.Project)]);
            ...
            NWDProject? tProject = NWDWebDataManager.GetDataByReference<NWDProject>(sHttpContext, tProjectReference);
            NWDPlayerClassConstruction? tOldObject = NWDWebDataManager.GetDataByReference<NWDPlayerClassConstruction>(sHttpContext, sObject.Reference);
    ...
    public override void DeleteAddon(NWDPlayerClassConstruction sObject, Dictionary<string, string> sValues, HttpContext sHttpContext)
    {
        if (sValues.ContainsKey(nameof(NWDProjectSubObject.Project)))
        {
            ulong tProjectReference = ulong.Parse(sValues[nameof(NWDProjectSubObject.Project)]);
