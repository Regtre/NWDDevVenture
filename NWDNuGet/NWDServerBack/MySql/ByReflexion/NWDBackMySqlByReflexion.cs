...
{
    public class NWDBackMySqlByReflexion : INWDDaoGeneric
    {
        ...
        public NWDBackMySqlByReflexion(NWDDatabaseCredentials sDatabaseCredentials)
        {
        ...
        private void CreateTable(NWDEnvironmentKind sEnvironment, Type sType)
        {
            ...
            {
                string tCanAdd = "SELECT COUNT(*) FROM `INFORMATION_SCHEMA`.`COLUMNS` WHERE `TABLE_SCHEMA`='" + _DbConnector.GetDatabaseName() + "' AND `TABLE_NAME`='" +
                                 TableName(sEnvironment, sType) + "' AND `COLUMN_NAME`='" + tFieldInfo.Name + "';";
                string tAdd = "ALTER TABLE `" + TableName(sEnvironment, sType) + "` ADD COLUMN `" + tFieldInfo.Name + "` " + PropertyInfoToSqlType(tFieldInfo) + ";";
                try
                {
                    int result = 0;

                    MySqlCommand tCmdTableCanAdd = new MySqlCommand(tCanAdd, tConn);
                    using (MySqlDataReader reader = tCmdTableCanAdd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                ...
                string tAlter = "ALTER TABLE `" + TableName(sEnvironment, sType) + "` MODIFY `" + tFieldInfo.Name + "` " + PropertyInfoToSqlType(tFieldInfo) + ";";
                try
                {
        ...
        public T GetFirstBy<T>(NWDEnvironmentKind sEnvironment, ulong sProjectId, Dictionary<string, string> sDictionary = null, string sAndWhere = "")
        ...
        public List<T> GetBy<T>(NWDEnvironmentKind sEnvironment, ulong sProjectId, Dictionary<string, string> sDictionary,
