...
{
    [NWDCustomPropertyDrawer(typeof(NWDReferencesAmount), true)]
    public class NWDReferencesAmountDrawer : NWDPropertyDrawer
    {
        ...
        private void Init (NWDSerializedProperty sProperty)
        {
            if (Info == null)
            {
                ...
                while (tType != null)
                {
                    if (tType.IsGenericType && tType.GetGenericTypeDefinition() == typeof(NWDReferencesAmount<>))
                    {
                ...
                {
                    NWDDictionaryPropertyDrawer tDrawer = new NWDDictionaryPropertyDrawer(typeof(ulong), new NWDReferencePropertyDrawer(ReferenceType), new NWDFloatPropertyDrawer());
